...
```
这两个算法其实是一样的，只是细节上处理的不一样。
代码一初始化了一个 `len + 1` 的dp数组，目的是为了设置`dp[0] = 0`, `dp[1] = nums[0]` ，但是代码二初始化的是`len`的dp数组，目的是为了设置`dp[0] = nums[0]`, `dp[1] = Math.max(nums[0], nums[1])`
